...
fun decimal(digits: List<Int>, base: Int): Int {
    ...
    for (i in 0..digits.size - 1)
        result += digits[i] * pow(base.toDouble(), digits.size - 1.0 - i).toInt()
    return result
}
...
fun decimalFromString(str: String, base: Int): Int {
    var list = mutableListOf<Int>()
    for (i in str) {
        if (i.toString().contains(Regex("""[a-z]"""))) list.add(i - 'a' + 10)
        else list.add(i - '0')
    }
...
fun russian(n: Int): String {
    ...
    val from10to19 = listOf(Pair(10, "десять"), Pair(11, "одиннадцать"), Pair(12, "двенадцать"), Pair(13, "тринадцать"),
            Pair(14, "четырнадцать"), Pair(15, "пятнадцать"), Pair(16, "шестнадцать"), Pair(17, "семнадцать"),
            Pair(18, "восемнадцать"), Pair(19, "девятнадцать"))
    ...
    if (result.isNotEmpty()) {
        when{
            numberList[4] != 1 && (numberList[3] == 2 || numberList[3] == 3 || numberList[3] == 4) -> result.add("тысячи")
            numberList[4] != 1 && numberList[3] == 1 -> result.add("тысяча")
